extends SceneTree
## Is the hero's contact shadow actually configured while the fight is
## running?
##
## Written when the contact shadow was a Decal that drew nothing: two A/B
## renders of the same gameplay frame came back pixel-identical around his feet,
## and "the node is not being driven" and "the renderer is not drawing it" want
## different fixes that a screenshot cannot tell apart. It read the decal back
## as entirely correct, which is what settled on the renderer and led to the
## quad that ships.
##
## Kept because that question recurs. It boots main.tscn for real, starts the
## fight, waits for the hero to land on the deck, and reads the shadow back:
## node type, visibility, world transform, material, and the ground under it.
##
## Run:
##   godot --headless --script res://scripts/players/_contact_shadow_probe.gd --path .
...
func _tick() -> void:
	...
	if not _started:
		...
		if menu == null or not menu.has_method("_on_activated"):
			if _steps > MAX_STEPS:
				print("[contact] MainMenu never appeared")
				quit()
			return   # the menu builds its own copy of the arena; wait for it
		menu.call("_on_activated", &"start")
		_started = true
		print("[contact] fight started via the menu at frame %d" % _steps)
		return
	if not _started:
		return
	...
	if hero == null:
		if _steps > MAX_STEPS:
			print("[contact] no node in group 'players' after %d frames" % _steps)
			quit()
		return
	if _found_at < 0:
		_found_at = _steps
		print("[contact] hero appeared at frame %d" % _steps)
		return
	if _steps - _found_at < SETTLE_STEPS:
		return

	print("[contact] hero=%s pos=%s on_floor=%s" % [
			hero.name, hero.global_position, hero.is_on_floor()])
	var d := hero.get_node_or_null("ContactShadow") as VisualInstance3D
	if d == null:
		print("[contact] ContactShadow NOT CREATED")
		quit()
		return

	print("[contact] node=%s visible=%s top_level=%s" % [d.get_class(), d.visible,
			(d as Node3D).top_level])
	var aabb := d.get_aabb()
	print("[contact] world pos=%s  aabb size=%s" % [d.global_position, aabb.size])
	var mi := d as MeshInstance3D
	if mi != null and mi.mesh != null:
		var mat := mi.mesh.surface_get_material(0) as StandardMaterial3D
		if mat != null:
			print("[contact] albedo=%s transparency=%d blend=%d unshaded=%s" % [
	...
	if hit.is_empty():
		print("[contact] ray from %s found NO ground on layer WORLD" % from)
	else:
		print("[contact] ground at y=%.3f  collider=%s" % [
